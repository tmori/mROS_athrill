...
mRosMemoryListEntryType* mros_protocol_topic_data_receive(mRosCommTcpClientType *client, mRosMemoryManagerType *mempool)
{
	...
	ret = mros_comm_tcp_client_receive_all(&client->socket, rawdata, MROS_TOPIC_RAWDATA_HEADER_SIZE, &res);
	if (ret != MROS_E_OK) {
		return NULL;
	}
	...
	ret = mros_comm_tcp_client_receive_all(&client->socket, packet.data, len, &res);
	if (ret != MROS_E_OK) {
		return NULL;
	}
	return mem_entryp;
